...
def test_valid_data():
    ...
    state, error = validate(john, template, parent=[])
    assert state == True
    assert error == ''
...
def test_missing_key():
    ...
    state, error = validate(eric, template, parent=[])
    assert state == False
...
def test_bad_type():
    ...
    state, error = validate(michael, template, parent=[])
    assert state == False
...
def test_extra_key():
    ...
    state, error = validate(graham, template, parent=[])
    assert state == False
...
def test_wrong_type_at_top_level():
    ...
    state, error = validate(terry_j, template, parent=[])
    assert state == False
...
def test_missing_top_level_key():
    ...
    state, error = validate(terry_g, template, parent=[])
    assert state == False
...
def test_additional_nested_key():
    ...
    state, error = validate(neil, template, parent=[])
    assert state == False
...
def test_empty_dictionary():
    # Empty data
    empty_data = {}
    state, error = validate(empty_data, template, parent=[])
    assert state == False
    assert error == 'mismatched keys: user_id'
...
def test_none_value():
    ...
    state, error = validate(carol, template, parent=[])
    assert state == False
...
def test_correct_types_but_extra_keys():
    ...
    state, error = validate(connie, template, parent=[])
    assert state == False
...
def test_wrong_type_in_nested_dict():
    ...
    state, error = validate(chapman, template, parent=[])
    assert state == False
